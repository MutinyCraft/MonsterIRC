...
import org.monstercraft.irc.plugin.command.gamecommands.Names;
...
import org.monstercraft.irc.plugin.command.gamecommands.Whois;
...
public class CommandManager {
	...
	private final LinkedList<IRCCommand> IRCCommands = new LinkedList<IRCCommand>();
	...
	public CommandManager() {
		...
		gameCommands.add(new Names());
		gameCommands.add(new Whois());
			...
			for (final GameCommand c : gameCommands) {
				if (c.canExecute(sender, split)) {
					...
					try {
						...
						final boolean b = c.execute(sender, split);
						if (!b) {
	...
	public boolean onIRCCommand(final String sender, final String arg,
		...
		for (final IRCCommand c : IRCCommands) {
