...
public enum ColorUtils {
	DARK_BLUE("\u00032", ChatColor.DARK_BLUE.toString()), DARK_GREEN("\u00033",
			ChatColor.DARK_GREEN.toString()), RED("\u00034", ChatColor.RED
			.toString()), BROWN("\u00035", ChatColor.DARK_RED.toString()), PURPLE(
			...
			"\u000314", ChatColor.DARK_GRAY.toString()), LIGHT_GRAY("\u000315",
	 ...
	 */
	ColorUtils(final String IRCColor, final String MinecraftColor) {
	...
	private static int find(final String message, final String main) {
		return message.indexOf(main);
	}

	private static String resolve(final String main, final String replacement,
			final int index) {
		final char[] chars = main.toCharArray();
	...
	private static String replace(final String input) {
		return input.replace("&", "§");
	}

	private static String resolve(final String input) {
		return input.replace("§", "&");
	}
